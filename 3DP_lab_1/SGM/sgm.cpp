...
namespace sgm {
  ...
  void SGM::compute_path_cost(int direction_y, int direction_x, int cur_y, int cur_x, int cur_path)
  {
    ...
    for(int d=0; d < disparity_range_; d++)
    {
      ...
      {
        //Please fill me!
        big_penalty_cost = path_cost_[cur_path][cur_y - direction_y][cur_x - direction_x][d];
        ...
        for(int pdisp = 0; pdisp < disparity_range_; pdisp++)
        {
            if((path_cost_[cur_path][cur_y][cur_x - direction_x][pdisp] + p2_) < big_penalty_cost)
                big_penalty_cost = path_cost_[cur_path][cur_y - direction_y][cur_x - direction_x][pdisp] + p2_;
        }
        path_cost_[cur_path][cur_y][cur_x][d] = cost_[cur_y][cur_x][d] + min(small_penalty_cost, big_penalty_cost) - best_prev_cost;

      }

      if(path_cost_[cur_path][cur_y][cur_x][d] < prev_cost)
          prev_cost = path_cost_[cur_path][cur_y][cur_x][d];
    }
